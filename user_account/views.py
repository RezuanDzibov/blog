...
from utilets.mixins import AddToContextMixin, UnLoginRequiredMixin
...
class UserProfileSearch(generic.list.ListView):
    ...
    def get_queryset(self):
        try:
            queryset = UserProfile.objects.annotate(search=SearchVector('user__username', 'user__first_name', 'user__last_name', 'user__email', 'bio')).filter(search=self.request.GET['q'])
...
class AddSocialNetworkLink(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            link = request.user.user_profile.social_links.get(social_net_name=kwargs['soc_net_name'])
            return shortcuts.redirect('user_account:my_profile')
    ...
    def get_success_url(self):
        self.success_url = reverse_lazy('user_account:profile', args=[self.request.user.username])
